...
const configurationSchema = z.object({
  ...
  general: z.object({
    ...
    messageApproveMethod: z
      ...
      .catch("Quick pick")
      .optional(),
    language: z.enum(["english", "french"]).default("english").catch("english"),
    emoji: z.enum(["true", "false"]).default("false").catch("false"),
    description: z.enum(["true", "false"]).default("false").catch("false"),
  }),
  openAI: z.object({
...
export async function setConfigurationValue(
  ...
  const configuration = vscode.workspace.getConfiguration("gptcommitSabrach");
